...
var mapControllEvent = function(magoInstance) {
    ...
    OIM.measureTools.lineShape = new Shape(2, false);
    OIM.measureTools.polygonShape = new Shape(3, false);
    ...
    selectInteraction.on(Mago3D.PointSelectInteraction.EVENT_TYPE.DEACTIVE, function(){
        ...
        OIM.selectedDataController.toggleWrap(false);
    });
    ...
    $('#mapCtrlMeasure').click(function() {
        ...
        if(OIM.measureTools.polygonShape.entityId.length !=0){
            OIM.measureTools.polygonShape.init();
        }
        if(OIM.measureTools.lineShape.entityId.length !=0){
            OIM.measureTools.lineShape.init();
        }
    });
    ...
    $('#mapCtrlMeasureDistance').click(function() {
        ...
        OIM.measureTools.lineShape.condition = function(){
            return $('#mapCtrlMeasureDistance')[0].classList.contains('on');
        };
        deleteHeigtEntities();
        OIM.measureTools.lineShape.setEventHandler();
    });
    ...
    $('#mapCtrlMeasureArea').click(function() {
        ...
        OIM.measureTools.polygonShape.condition = function(){
            return $('#mapCtrlMeasureArea')[0].classList.contains('on');
        };
        deleteHeigtEntities();
        OIM.measureTools.polygonShape.setEventHandler();
    });
    ...
    $('#mapCtrlSetting').click(function() {
        ...
        if ($(this).hasClass('on')) {
            if(OIM.selectedDataController.isActive()) {
                $('#dataControlWrap').css('right', '0px');
            } else {
                $('#mapSettingWrap').css('right', '0px');
            }
            ...
            $('#mapSettingWrap').css('right', '-340px');
            $('#dataControlWrap').css('right', '-340px');
    ...
    magoInstance.getViewer().clock.onTick.addEventListener(function(clock) {
        if(heigtEntities.length > 0 && heigtEntities.length === 3) {
            ...
            var position = point.position.getValue(clock.currentTime);
