...
$(document).ready(function (){
	...
	$("#layerContent").on("click", '.layerGroup', function(e){
		...
		if($(this).text() === "OFF") {
			...
			OIM.map.addGroupLayer(Number(layerGroupId));
		} else {
			$(this).text("OFF");
			$('.nodepth').each(function(e){
				if(layerGroupId === $(this).attr("data-ancestor")) {
			...
			OIM.map.removeGroupLayer(Number(layerGroupId));
		}
	});
...
function turnOnAllLayer() {
	...
	$('.nodepth').addClass("on");
	$('.layerGroup').text("ON");
	OIM.map.removeAllLayers();
	OIM.map.initLayer(true);
}
//레이어 전체 끄기
